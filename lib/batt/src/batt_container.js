module.exports = function(parameters) {
	$.extend(this, {
		...
		*/
		...
		findType: function(type, context) {
			if (!context)
				context = this;
			...
			var found = [];
			if (context.type == type) // Include self?
				found.push(context);

			for (var c in context.children) {
				if (context.children[c].children) { // Has its own children
					var foundChildren = context.findType(type, context.children[c]);
					if (foundChildren)
						found = found.concat(foundChildren);
				} else if (context.children[c].type == type) // No children - probably a simple object
					found.push(context.children[c]);
		...
		*/
		...
		setup: function() {
			this.element = $(this.parse(this.renderTag));
			this.elementContainer = this.renderSelector ? this.element.find(this.renderSelector) : this.element;
			return this;
		},
		...
		*/
				...
				me.loadContainerData(function() {
					...
					while (data = me.getData()) {
						...
						for (var i in me.childrenOrder) {
							...
							me.renderRow(me.elementContainer, child);
						}
					}
					me.status = 'idle';
					me.change();
		...
		clear: function() {
			this.elementContainer.children(':not(.batt-protected)').detach();
			return this;
		},
