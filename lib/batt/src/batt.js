...
global.batt = {
	...
	* @param function success Callback function to run when loaded
	*/
	plugin: function(name, success) {
		if (typeof name == 'array') { // Given an array - overload to individual calls
			for (var p in name)
				batt.plugin(name[p]);
		} else {
			console.log('Batt> Loading plugin ', name);
			$script(batt.path + '/plugins/' + name + '.js', success);
		}
	},
	...
	*/
	...
	find: function(id) {
		if (!id) {
			console.warn('batt.find() - Asked to find a null element!');
			return [];
		}
		if (id.substr(0, 1) == '#') {
			return this.findId(id.substr(1));
		} else if (id.substr(0, 1) == '.') {
			return this.findType(id.substr(1));
		} else { // Default to just looking for the ID
	...
	findType: function(type) {
		var found = [];
		for (var f in batt.forms) { // Recurse into each form
			var foundChildren = batt.forms[f].findType(type);
			if (foundChildren)
	...
	*/
	...
	// Global event/hook functions {{{
	events: {},
	on: function(binding, func) {
		if (!this.events[binding])
			this.events[binding] = [];
		this.events[binding].push(func);
		return this;
	},
	...
	trigger: function(binding) {
		...
		if (this.events[binding])
			for (var e in this.events[binding])
				this.events[binding][e].call(this);
		return this;
	},
	// }}}
	// Special functions {{{
