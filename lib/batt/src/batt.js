		...
		if (!form) { // No specific specified - go to first found
			...
			for (var f in $.batt.forms) {
				$.batt.forms[f].eachChild(function() {
					if (this.type == 'form') { // Found a form!
						if (!firstForm) { // Not yet seen a form
							firstForm = this;
						} else { // Seen at least one other form - hide from here
							this.hide();
						}
					}
				}, {depthFirst: true});
				firstForm.eachParent(function() {
					this.show();
				}, {andSelf: true});
			}
		} else { // Switch to a specific form
			$.batt.find(form)
				.eachParent(function() {
